...
**Prerequisites**: A way to track tasks (any approach works). No existing epic for the topic.
...
4. `/sprint` — Reads the plan and populates backlog tasks. Dispatches agents to execute the plan.
...
| `/plan` | `/sprint` | Context — sprint reads the plan and translates to backlog tasks |
