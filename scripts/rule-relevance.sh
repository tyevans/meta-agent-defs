...
#
# Works in both plugin context (CLAUDE_PLUGIN_ROOT set) and
# symlink install context (rules at CWD/rules/).
...
# Search global rules (~/.claude/rules/) and project-local rules (.claude/rules/)
RELEVANT=""
for rule_file in "${HOME}/.claude/rules/"*.md "${PWD}/.claude/rules/"*.md; do
